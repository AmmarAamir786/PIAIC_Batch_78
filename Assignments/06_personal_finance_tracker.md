...
### ⚠️ CRITICAL: Understanding the Folder Structure

**READ THIS CAREFULLY. Students make mistakes here ALL THE TIME.**

Your project will have **TWO SEPARATE FOLDERS** — `backend/` and `frontend/`. They are **NOT nested inside each other**. They are **siblings** (both at the same level).

```
✅ CORRECT STRUCTURE:
personal-finance-tracker/          ← Root folder (where you open VS Code)
...
│   ├── package.json
│   └── node_modules/
├── SPEC.md                        ← Generated by Claude Code
├── README.md
└── .gitignore

❌ WRONG STRUCTURE (do NOT do this):
frontend/
└── backend/                       ← NO! Backend is NOT inside frontend!

❌ WRONG STRUCTURE (do NOT do this):
backend/
└── frontend/                      ← NO! Frontend is NOT inside backend!
```

**Why does this matter?**

1. **Different runtimes:** Backend runs Python (`uv run`), frontend runs Node.js (`npm run dev`). They need to be separate.
2. **Different dependencies:** Backend has `pyproject.toml` + `.venv`, frontend has `package.json` + `node_modules`. Mixing them causes chaos.
3. **Terminal navigation:** When running commands, you `cd backend` or `cd frontend` **from the root folder**, not from inside each other.

**When you run commands:**

```bash
# ALWAYS start from the root folder: personal-finance-tracker/
...
# To run frontend (in a different terminal):
cd frontend             ← Go INTO the frontend folder
npm run dev
cd ..                   ← Go BACK to root
```

**⚠️ If Claude Code creates the wrong structure, STOP and fix it immediately. Don't continue building on top of a broken structure.**
...
##### Step 0.2d: Create a GitHub Repository and Push Your Initial Setup

**⚠️ IMPORTANT:** You MUST create a GitHub repository for this project. This is required for submission, and it's a best practice to commit your work regularly.

**Step 1: Create `.gitignore` FIRST (before your first commit)**

In the root `personal-finance-tracker/` folder, create a `.gitignore` file with this content:
...
**Why `.gitignore`?** This file tells Git which files to NEVER track or upload. Your `.env` files contain passwords and secrets — if you upload them to GitHub, anyone can steal your database.

**Step 2: Initialize Git and create your first commit**

```bash
# Make sure you're in the root personal-finance-tracker/ folder
git init
git add .
git commit -m "Initial setup: backend (FastAPI + uv) and frontend (Next.js 16)"
```

**Step 3: Create a GitHub repository**

1. Go to [https://github.com/new](https://github.com/new)
2. Repository name: `personal-finance-tracker` (or any name you like)
3. Description: "Full-stack personal finance tracker built with Next.js 16 and FastAPI"
4. **Keep it Public** (so instructors can see it)
...
**🎉 Your project is now on GitHub!** Visit the URL to see it online.

**✅ CHECKPOINT:** After every major verified working change, commit and push:
...
Examples of "major verified working changes":
- Backend API is working (tested via `/docs`)
- Frontend dashboard displays correctly
- All backend tests pass
- Bug fix that you've verified works

**💡 TIP:** Once your backend and frontend are set up and verified working (Phase 1 and Phase 2 complete), push to GitHub. This creates a checkpoint. If something breaks later, you have a working version to fall back to.
...
After completing and verifying each major phase (e.g., Phase 1: Backend complete and tested), **commit and push to GitHub:**

```bash
git add .
git commit -m "Complete Phase 1: Backend API with all CRUD endpoints and summary"
git push
```

**Why?** This creates save points. If Phase 2 breaks something, you have a working Phase 1 to go back to. Think of it like saving your game progress.

**Recommended commit points:**
- After Phase 1 (Backend complete, all endpoints tested via `/docs`)
- After Phase 2 (Frontend complete, UI displays correctly)
- After Phase 3 (Full integration working, data flows correctly)
- After Phase 4 (All tests written and passing)
- After fixing any major bug (once you verify the fix works)
...
1. ✅ **Complete source code** (both `backend/` and `frontend/` folders in the correct structure)
...
2. **Commit and push after every major verified change.** After Phase 1 works, commit. After Phase 2 works, commit. After fixing a bug, commit. This creates checkpoints you can revert to if something breaks.
3. **Check the folder structure immediately.** After Claude creates files, verify the structure is correct: `backend/` and `frontend/` as **siblings**, not nested. If wrong, stop and fix it.
4. **Test after each phase.** After each phase is complete in SPEC.md, verify it works before letting Claude move on. It's much harder to debug everything at the end.
5. **If Claude loses context, say:** "Read SPEC.md and continue from where we left off." This is the magic of spec-driven development — Claude can always resume.
6. **Understand what Claude Code generates.** After Claude generates code, read through it and make sure you can explain what each part does. If you don't understand something, ask Claude Code to explain it.
7. **Use `/docs` to test your backend.** Before building any frontend, make sure your API works by testing it through FastAPI's built-in documentation page.
8. **Keep your terminal open.** Watch for errors in both the backend and frontend terminals. Error messages tell you exactly what's wrong.
9. **Check SPEC.md regularly.** It's your progress dashboard. Glance at it to see which tasks are done and which are next.
10. **Use `uv run` for everything Python.** Never run `python` or `pip` directly. Always use `uv run python ...`, `uv run pytest`, etc. This ensures you're always using the correct virtual environment.
...
| ----------------------------------------------- | -------------------------------------------- | ---------------------------------------------------------- |
| **Wrong folder structure**                | Not understanding backend/frontend are separate | Check structure immediately. They must be siblings, NOT nested |
| **No GitHub repo or commits**             | Forgetting to initialize Git                 | Create repo FIRST (Step 0.2d), commit after every phase   |
| **Pushing `.env` to GitHub**            | Missing `.gitignore` or wrong setup        | Create `.gitignore` BEFORE first commit (see Step 0.2d) |
